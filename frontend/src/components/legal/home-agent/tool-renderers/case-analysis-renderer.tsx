...
export function CaseAnalysisRenderer({ data, caseId }: Props) {
  ...
  if (!d || !d.summary) {
    return <p className="text-[15px] text-muted-foreground">분석 결과가 없습니다.</p>;
  }

  const sections = [
    { icon: FileText, label: "요약", content: d.summary, color: "#6D5EF5" },
    { icon: AlertCircle, label: "사실 관계", content: d.facts, color: "#F59E0B" },
      ...
      {(d.crime_names.length > 0 || d.legal_keywords.length > 0) && (
        <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-border/30">
            ...
            <span className="text-[15px] font-semibold text-foreground">관련 키워드</span>
          </div>
          <div className="px-3 py-2.5 flex flex-wrap gap-1.5">
            {d.crime_names.map((name) => (
              <span key={name} className="text-[12px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 font-medium">
                {name}
              </span>
            ))}
            {d.legal_keywords.map((kw) => (
              <span key={kw} className="text-[12px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 font-medium">
                {kw}
              </span>
            ))}
          </div>
